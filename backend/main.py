...
async def handle_request(request: Request):
    ...
    intent = payload['queryResult'].get('intent', {}).get('displayName')
    ...
    if not intent or not output_contexts:
        return JSONResponse(content={"fulfillmentText": "Invalid request. Missing required fields."})
    ...
    if intent in intent_handler_dict:
        ...
        return JSONResponse(content={"fulfillmentText": "Unknown intent."})
...
def remove_from_order(parameters: dict, session_id: str):
    ...
    if not food_items:
        return JSONResponse(content={
            "fulfillmentText": "No food items provided to remove."
        })
...
def track_order(parameters: dict, session_id: str):
    # Check for both 'order_id' and 'number' in the parameters
